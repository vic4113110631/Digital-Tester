...
// interrupt
LPPCIE_IntIsEnabled PCIE_IntIsEnabled;
LPPCIE_IntEnable	PCIE_IntEnable;
LPPCIE_IntDisable	PCIE_IntDisable;
...
void *PCIE_Load(void){
	...
	if(bSuccess){
		...
		// interrupt
		PCIE_IntIsEnabled = (LPPCIE_IntIsEnabled)dlsym(lib_handle, "PCIE_IntIsEnabled");
		PCIE_IntEnable = (LPPCIE_IntEnable)dlsym(lib_handle, "PCIE_IntEnable");
		PCIE_IntDisable = (LPPCIE_IntDisable)dlsym(lib_handle, "PCIE_IntDisable");
		...
		if (!PCIE_Open || !PCIE_Close ||
			...
			!PCIE_IntIsEnabled || !PCIE_IntEnable ||
			!PCIE_IntDisable 
		)
			bSuccess = FALSE;
